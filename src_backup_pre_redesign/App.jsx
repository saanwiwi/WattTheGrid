      ...
      <>
        <div className="border-r border-white/20 p-4 flex flex-col justify-between transition-colors group relative overflow-hidden">
          <div className="flex justify-between items-center">
            ...
            <span className="font-mono text-[7px] text-[#FFEA00] bg-[#FFEA00]/10 border border-[#FFEA00]/30 px-1 py-0.5 font-bold">96.4% EFF</span>
          </div>
        ...
        <div className="border-r border-white/20 p-4 flex flex-col justify-between transition-colors group relative overflow-hidden">
          <div className="flex justify-between items-center">
            ...
            <span className="font-mono text-[7px] text-[#1434FB] bg-[#1434FB]/10 border border-[#1434FB]/30 px-1 py-0.5 font-bold">4 UNITS</span>
          </div>
          <div className="flex justify-between items-end mt-2">
        ...
        <div className="border-r border-white/20 p-4 flex flex-col justify-between transition-colors group relative overflow-hidden">
          <div className="flex justify-between items-center">
            ...
            <span className="font-mono text-[7px] text-[#FF2A2A] bg-[#FF2A2A]/10 border border-[#FF2A2A]/30 px-1 py-0.5 font-bold">LFP_CELL</span>
          </div>
          <div className="flex justify-between items-end mt-2">
            <div className="flex flex-col">
              ...
              <div className="flex gap-0.5 mt-1">
                {Array.from({ length: 8 }).map((_, i) => (
                  <span
                    key={i}
                    className={`h-1.5 w-1 ${i < Math.round((history[history.length-1].bessSoc / 100) * 8) ? 'bg-[#FF2A2A]' : 'bg-white/10'}`}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="p-4 flex flex-col justify-between transition-colors group relative overflow-hidden">
          <div className="flex justify-between items-center">
            ...
            <span className="font-mono text-[7px] text-white/60 bg-white/10 border border-white/20 px-1 py-0.5 font-bold">60.0Hz</span>
          </div>
          <div className="flex justify-between items-end mt-2">
            <span className="text-2xl font-black font-mono text-white">200<span className="text-xs ml-1 opacity-50">kW</span></span>
            <div className="flex items-end gap-1 h-6 pb-1 opacity-70">
              <span className="w-1 bg-white/40 h-3 animate-pulse" style={{ animationDelay: '0ms' }} />
              <span className="w-1 bg-white/80 h-5 animate-pulse" style={{ animationDelay: '180ms' }} />
              <span className="w-1 bg-white/60 h-4 animate-pulse" style={{ animationDelay: '360ms' }} />
              <span className="w-1 bg-white/90 h-6 animate-pulse" style={{ animationDelay: '120ms' }} />
              <span className="w-1 bg-white/40 h-2 animate-pulse" style={{ animationDelay: '480ms' }} />
            </div>
          </div>
        </div>
      </>
      ...
      <div className={`min-h-screen bg-[#09070D] text-[#F7F7F2] font-sans selection:bg-[#FFEA00] selection:text-black flex flex-col overflow-hidden brutalist-grid ${isCrisis ? 'crisis-vignette' : ''}`} style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 h-full">
          ...
          <div className="col-span-5 border-r border-white/20 flex flex-col">
            <div className={`flex-grow flex flex-col justify-center relative p-8 transition-colors duration-500 overflow-hidden ${isCritical ? 'bg-[#FF2A2A]/20' : 'bg-transparent'}`}>
              {/* Corner brackets */}
              <div className="absolute top-4 left-4 font-mono text-xs text-white/20 select-none pointer-events-none">┌</div>
              <div className="absolute top-4 right-4 font-mono text-xs text-white/20 select-none pointer-events-none">┐</div>
              <div className="absolute bottom-20 left-4 font-mono text-xs text-white/20 select-none pointer-events-none">└</div>
              <div className="absolute bottom-20 right-4 font-mono text-xs text-white/20 select-none pointer-events-none">┘</div>

              <div className="absolute top-6 left-6 font-mono text-[10px] tracking-widest text-white/50 uppercase flex gap-4 z-10">
                <span>[ TX-400 MAIN BUS LOAD ]</span>
                {WEBSOCKET_MODE && <span className="text-[#FFEA00] animate-pulse">WS CONNECTED</span>}
              </div>
              
              {/* Physical Electromagnetic Energy Field Canvas Layer */}
              <EnergyField load={load} isCrisis={isCrisis} />
              ...
              <div
                ...
                style={{
                  background: isCritical
                    ? 'radial-gradient(ellipse 55% 35% at 50% 52%, rgba(255,42,42,0.12) 0%, transparent 70%)'
                    : 'radial-gradient(ellipse 55% 35% at 50% 52%, rgba(20,52,251,0.10) 0%, transparent 70%)',
                }}
              />
              <div className="flex items-start justify-center mt-12 relative z-10">
                <span
                  className={`text-[clamp(8rem,14vw,18rem)] font-black tracking-tighter leading-none transition-all duration-300 select-none ${isCritical ? 'text-[#FF2A2A]' : 'text-white'}`}
                  style={{ textShadow: isCritical ? '0 0 60px rgba(255,42,42,0.25)' : '0 0 60px rgba(255,255,255,0.08)' }}
                >
                  {load}
                </span>
                <span className="text-2xl font-mono text-[#1434FB] mt-8 ml-2 font-bold select-none">kW</span>
              </div>

              <div className="text-center font-mono text-[9px] tracking-[0.25em] text-white/40 uppercase mb-2 relative z-10">
                SYS_FREQ: 49.98 Hz // IMPEDANCE: 0.042 Ω // STATUS: {isCritical ? 'ALERT_SURGE' : 'PHASE_LOCKED'}
              </div>
                ...
                <div style={{ transform: 'rotateX(60deg) rotateZ(-45deg)', transformStyle: 'preserve-3d' }} className="relative w-48 h-48 transition-transform duration-700 ease-in-out hover:rotate-x-[55deg] hover:rotate-z-[-40deg]">
                  ...
                  <svg className="absolute inset-0 w-full h-full overflow-visible" style={{ pointerEvents: 'none', zIndex: 0 }}>
                    <line x1="50%" y1="50%" x2="50%" y2="-4"
                      stroke={(activeNode === 'SOLAR' || hoveredNode === 'SOLAR') ? '#FFEA00' : 'rgba(255,234,0,0.22)'}
                    ...
                    <line x1="50%" y1="50%" x2="104%" y2="50%"
                      stroke={(activeNode === 'EV' || hoveredNode === 'EV') ? '#1434FB' : 'rgba(20,52,251,0.25)'}
                    ...
                    <line x1="50%" y1="50%" x2="50%" y2="104%"
                      stroke={(activeNode === 'BESS' || hoveredNode === 'BESS') ? '#FF2A2A' : 'rgba(255,42,42,0.25)'}
                    ...
                    <circle cx="50%" cy="0%" r="3" fill="#FFEA00" className="energy-dot-solar" />
                    <circle cx="50%" cy="50%" r="3" fill="#1434FB" className="energy-dot-ev" />
                    <circle cx="50%" cy="50%" r="3" fill="#FF2A2A" className="energy-dot-bess" />
                  </svg>
                  ...
                  {(crisisStep === 'INTERVENTION' || crisisStep === 'STABILIZED') && (
                    <svg className="absolute inset-0 w-full h-full z-0 overflow-visible">
                      ...
                      <line x1="50%" y1="100%" x2="100%" y2="50%" stroke="#FFEA00" strokeWidth="3.5" strokeDasharray="6 6" style={{ animation: 'dash 0.4s linear infinite' }} />
                      <circle cx="50%" cy="100%" r="3.5" fill="#FFEA00" className="energy-dot-p2p" />
                    </svg>
                  )}
                  
                  {/* Floating Nodes — with breathing glow, hover response & telemetry labels */}
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex flex-col items-center">
                    <span className="font-mono text-[7px] text-[#FFEA00] tracking-wider whitespace-nowrap opacity-80 pointer-events-none mb-1">PV_180kW</span>
                    <button
                      ...
                      className={`w-10 h-10 border-2 ${activeNode === 'SOLAR' ? 'border-white bg-[#FFEA00]' : 'border-[#FFEA00] bg-[#09070D]'} ${(activeNode === 'SOLAR' || hoveredNode === 'SOLAR') ? 'scale-110 shadow-[0_0_18px_rgba(255,234,0,0.6)]' : ''} rounded-none flex items-center justify-center transition-all duration-300 cursor-pointer z-10 node-breath-yellow`}
                    >
                  ...
                  <div className="absolute top-1/2 -right-4 -translate-y-1/2 flex items-center">
                    <button
                      ...
                      onMouseLeave={() => setHoveredNode(null)}
                      style={{ transform: 'rotateZ(45deg) rotateX(-60deg) translateZ(20px)' }}
                      className={`w-10 h-10 border-2 ${activeNode === 'EV' ? 'border-white bg-[#1434FB]' : 'border-[#1434FB] bg-[#09070D]'} ${(activeNode === 'EV' || hoveredNode === 'EV') ? 'scale-110 shadow-[0_0_18px_rgba(20,52,251,0.6)]' : ''} rounded-none flex items-center justify-center transition-all duration-300 cursor-pointer z-10 node-breath-blue`}
                    >
                      <Car className={`w-5 h-5 ${activeNode === 'EV' ? 'text-white' : 'text-[#1434FB]'}`} />
                    </button>
                    <span className="font-mono text-[7px] text-[#1434FB] tracking-wider whitespace-nowrap opacity-80 pointer-events-none ml-2">DEPOT_V2G</span>
                  </div>
                  ...
                  <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center">
                    <button
                      ...
                      onMouseLeave={() => setHoveredNode(null)}
                      style={{ transform: 'rotateZ(45deg) rotateX(-60deg) translateZ(20px)' }}
                      className={`w-10 h-10 border-2 ${activeNode === 'BESS' ? 'border-white bg-[#FF2A2A]' : 'border-[#FF2A2A] bg-[#09070D]'} ${(activeNode === 'BESS' || hoveredNode === 'BESS') ? 'scale-110 shadow-[0_0_18px_rgba(255,42,42,0.6)]' : ''} rounded-none flex items-center justify-center transition-all duration-300 cursor-pointer z-10 node-breath-red`}
                    >
                      <Battery className={`w-5 h-5 ${activeNode === 'BESS' ? 'text-white' : 'text-[#FF2A2A]'}`} />
                    </button>
                    <span className="font-mono text-[7px] text-[#FF2A2A] tracking-wider whitespace-nowrap opacity-80 pointer-events-none mt-1">BESS_98%</span>
                  </div>

                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                    <span className="font-mono text-[7px] text-white tracking-widest whitespace-nowrap opacity-90 font-black pointer-events-none mb-1">TX-400</span>
                    <button
                      onClick={() => setActiveNode('GRID')}
                      onMouseEnter={() => setHoveredNode('GRID')}
                      onMouseLeave={() => setHoveredNode(null)}
                      style={{ transform: 'rotateZ(45deg) rotateX(-60deg) translateZ(40px)' }}
                      className={`w-14 h-14 border-4 flex items-center justify-center z-20 transition-all duration-500 cursor-pointer node-breath-white ${isCritical ? 'border-[#FF2A2A] scale-125 bg-[#FF2A2A]/20' : activeNode === 'GRID' ? 'border-[#FFEA00] bg-white/10' : 'border-white bg-[#09070D]'} ${hoveredNode === 'GRID' ? 'scale-110 shadow-[0_0_24px_rgba(255,255,255,0.5)]' : ''}`}
                    >
