...
export default function EnergyField({ load = 120, isCrisis = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Micro Field Readout Faint Text in corners
      ctx.font = '8px "JetBrains Mono", monospace';
      ctx.fillStyle = `rgba(255, 255, 255, ${isOverload ? 0.5 : 0.22})`;
      ctx.fillText(`FLUX: ${(load * 1.84).toFixed(1)} G`, margin + 6, margin + 26);
      ctx.fillText(`HARMONIC: ${isOverload ? 'ERR_THD_HIGH' : 'THD 1.08%'}`, margin + 6, margin + 38);
      ...
      ctx.fillText(rightText, canvas.width - margin - textWidth - 6, margin + 26);
